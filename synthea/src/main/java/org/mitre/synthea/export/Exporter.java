...
public abstract class Exporter {
  ...
  // Thread-local buffering to reduce lock contention
  private static final int THREAD_BUFFER_SIZE = 512 * 1024; // 512KB per thread buffer
  private static final ThreadLocal<ConcurrentHashMap<Path, StringBuilder>> threadBuffers =
   ...
   * Uses thread-local buffering to reduce lock contention.
   * @param file Path to the new file.
   * @param contents The contents of the file.
   */
  public static void appendToFile(Path file, String contents) {
    // Get this thread's buffer map
    ConcurrentHashMap<Path, StringBuilder> buffers = threadBuffers.get();
    StringBuilder buffer = buffers.computeIfAbsent(file, k -> new StringBuilder(THREAD_BUFFER_SIZE));

    // Append to thread-local buffer
    buffer.append(contents).append('\n');

    // Flush if buffer is getting large
    if (buffer.length() >= THREAD_BUFFER_SIZE) {
      flushThreadBuffer(file, buffer);
    }
  }

  /**
   * Flush a thread-local buffer to disk.
   * @param file Path to the file
   * @param buffer The buffer to flush
   */
  private static void flushThreadBuffer(Path file, StringBuilder buffer) {
    if (buffer.length() == 0) {
      return;
    }
  ...
  /**
   * Flush all thread-local buffers to disk.
   * Should be called at the end of generation to ensure all data is written.
   */
  public static void flushAllThreadBuffers() {
    ConcurrentHashMap<Path, StringBuilder> buffers = threadBuffers.get();
    for (Map.Entry<Path, StringBuilder> entry : buffers.entrySet()) {
      flushThreadBuffer(entry.getKey(), entry.getValue());
    }
  }
