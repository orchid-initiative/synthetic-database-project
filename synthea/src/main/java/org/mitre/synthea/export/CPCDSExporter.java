...
public class CPCDSExporter {
   ...
   */
  private BufferedWriter patients;
   ...
   */
  private BufferedWriter claims;

  /**
   * Writer for Hospitals.csv
   */
  private BufferedWriter hospitals;

  /**
   * Writer for Practitioners.csv
   */
  private BufferedWriter practitioners;

  /**
   * Lock for thread-safe access to all file writers
   */
  private final Object writerLock = new Object();

  /**
   * Thread-local Random instance to avoid expensive Random() creation
   */
  ...
  private CPCDSExporter() {
    try {
      ...
      coverages = new BufferedWriter(new FileWriter(coverageFile, append), 4 * 1024 * 1024);
      patients = new BufferedWriter(new FileWriter(patientsFile, append), 4 * 1024 * 1024);
      claims = new BufferedWriter(new FileWriter(claimsFile, append), 4 * 1024 * 1024);
      hospitals = new BufferedWriter(new FileWriter(hospitalFile, append), 4 * 1024 * 1024);
      practitioners = new BufferedWriter(new FileWriter(practitionerFile, append), 4 * 1024 * 1024);
   ...
   */
  private void write(String line, BufferedWriter writer) throws IOException {
    synchronized (writerLock) {
      writer.write(line);
    }
  }
